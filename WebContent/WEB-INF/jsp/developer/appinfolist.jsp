...
<div class="row">
	<div class="col-md-12">
		<div class="x_panel">
			...
			<div class="x_content">
				<form method="post"
					action="${pageContext.request.contextPath }/appinfo/list">
					...
					<ul>
									...
									<select id="queryCategoryLevel1" name="queryCategoryLevel1"
										...
										<c:if test="${categoryLevel1List != null }">
											...
											<c:forEach var="appCategory" items="${categoryLevel1List}">
												<option
													<c:if test="${appCategory.id == categoryLevel1}">selected="selected"</c:if>
						...
						<li>
							<div class="form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<input type="hidden" value="${categoryLevel2}" id="pid" /> <input
										type="hidden" name="categorylevel2list"
										...
										<c:if test="${categoryLevel2Lists != null }">
											...
											<c:forEach var="appCategory" items="${categoryLevel2Lists}">
												<option
													<c:if test="${appCategory.id == categoryLevel2}">selected="selected"</c:if>
													value="${appCategory.id}">${appCategory.categoryName}</option>
											</c:forEach>
										</c:if>
									</select>
						...
						<li>
							<div class="form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<input type="hidden" value="${categoryLevel3}" id="pid3" /> <select
										name="queryCategoryLevel3" id="queryCategoryLevel3"
										class="form-control">
										<c:if test="${categoryLevel3Lists != null }">
											<option value="0">--请选择--</option>
											<c:forEach var="appCategory" items="${categoryLevel3Lists}">
												<option
													<c:if test="${appCategory.id == categoryLevel3 }">selected="selected"</c:if>
													value="${appCategory.id}">${appCategory.categoryName}</option>
											</c:forEach>
										</c:if>
									</select>
		...
		<div class="x_panel">
			<div class="x_content">
				...
				<div id="datatable-responsive_wrapper"
					...
					<div class="row">
						<div class="col-sm-12">
							<a href="${pageContext.request.contextPath}/appinfo/appinfoadd"
									...
									<c:forEach var="appInfo" items="${appInfoList }"
										...
										<tr role="row" class="odd">
											...
											<td>${appInfo.categoryLevel1Name }->
												${appInfo.categoryLeve21Name} ->
												${appInfo.categoryLeve31Name }</td>
														...
														<li><c:choose>
																<c:when
																	...
																	<a class="saleSwichOpen" saleSwitch="open"
																		...
																		data-toggle="tooltip" aa="上架" data-placement="top"
																		title=""
																...
																<c:when test="${appInfo.status == 4}">
																	<a class="saleSwichClose" saleSwitch="close"
																		appinfoid=${appInfo.id }
																		appsoftwarename=${appInfo.softwareName }
																		data-toggle="tooltip" aa="下架" data-placement="top"
																		title=""
